...
// ORDER-PRESERVING rename that keeps the key in the same position
/**Why this works: instead of {...obj, [newKey]: value} (which pushes the new key to the end), we rebuild via Object.fromEntries() and replace the key inline, so its position doesn’t change. */
function renameKeyAtPathOrdered(
  root: JsonValue,
  pathToParent: Path,
  oldKey: string,
  newKey: string
  ...
  const parentObj = parent as JsonObject;
  if (oldKey === newKey || !(oldKey in parentObj)) return root;

  // Resolve conflict by suffixing _1, _2, ...
  let targetKey = newKey;
  if (targetKey in parentObj && targetKey !== oldKey) {
    let i = 1;
    while (`${newKey}_${i}` in parentObj) i += 1;
    targetKey = `${newKey}_${i}`;
  }

  // Rebuild object from entries, swapping only the key — preserves order
  const ordered = Object.fromEntries(
    Object.entries(parentObj).map(([k, v]) =>
      k === oldKey ? [targetKey, v] : [k, v]
    )
  ) as JsonObject;

  return setAtPath(root, pathToParent, ordered);
}
      ...
      {canEditKey ? (
        <div className="flex items-center gap-2">
          {editing ? (
            ...
            <div className="flex items-center gap-2">
              <Label className="text-sm font-medium text-muted font-mono">
                {name}
              </Label>
              <Button
                size="sm"
        ...
        <Label className="text-sm font-medium uppercase text-accent">
          {name.replaceAll("_", " ")}
        </Label>
      )}
    </div>
...
const PrimitiveEditor: React.FC<PrimitiveEditorProps> = ({
    ...
    return (
      <div className="flex items-center gap-3 py-1">
        ...
        <span className="text-sm text-muted-foreground">
          {/* {String(value)} */}
          {String(value) === "true" ? "Verdadero" : "Falso"}
        </span>
      </div>
    );
  }
  if (t === "number") {
    return (
      <Input
        type="number"
        // TODO: Force the change to be zero if needed but never to a string to avoid the type from changing to string
  ...
  return (
    <div className="space-y-3">
      {keys.map((k) => {
        ...
        return (
          <Card key={k} className="not-border-muted/50">
            <CardHeader className="py-3">
              <div className="flex items-center justify-between">
                <FieldHeader
                  ...
                  onRename={(nextName) =>
                    setData(renameKeyAtPathOrdered(data, path, k, nextName))
                  }
                />
                {structureMode && (
                  <Button
...
export const JsonFormEditor: React.FC<JsonFormEditorProps> = ({
  ...
  const [structureMode, setStructureMode] = useState<boolean>(false);
      ...
      <Card>
        ...
        <CardContent className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span
              // className={`text-sm ${structureMode ? "text-accent" : ""}`}
              className={`text-sm text-accent`}
            >
              {/* {structureMode ? "Enabled" : "Disabled"} */}
              {structureMode ? "Habilitado" : "Deshabilitado"}
            </span>
          </div>
        </CardContent>
      </Card>
