...
export async function POST(request: Request) {
  try {
    const { functionName, payload, invocationType }: LambdaRequestBody =
      await request.json()
    ...
    const rawPayload = response.Payload
      ? textDecoder.decode(response.Payload)
      : ''
    ...
    if (response.FunctionError) {
      const errorMessage =
        extractErrorMessage(parsedPayload) ?? response.FunctionError
