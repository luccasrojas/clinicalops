...
1. **Document existing structure**
   - [x] Capture how the editor bootstraps its JSON, how the `ClinicalHeading`
     and `ClinicalText` extensions enforce hierarchy, and how autosave talks to
     the Lambda.
   - [x] Map where `floating-block-toolbar.tsx` is mounted so that we can
     replace it with a block-based toolbar.

2. **Block action toolbar (frontend)**
   - [x] Replace the current floating toolbar so that it appears whenever the
     caret is inside a block (no selection required) and shows `+ Texto`,
     `+ Subtítulo`, `+ Subsección`, and `Eliminar bloque`.
   - [x] Implement insertion helpers that drop the new block immediately after
